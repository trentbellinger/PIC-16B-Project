...
df_unique = df.drop_duplicates(subset='ORIGIN')
df_renamed = df_unique.rename(columns={'ORIGIN_LATITUDE': 'lat', 'ORIGIN_LONGITUDE': 'lon'})
airport_coordinates = df_renamed.set_index('ORIGIN')[['lat', 'lon']].to_dict(orient='index')

route_delays = df[['ORIGIN', 'DEST', 'ORIGIN_LATITUDE', 'ORIGIN_LONGITUDE', 'DEST_LATITUDE', 'DEST_LONGITUDE', 'DEP_DEL15']].groupby(['ORIGIN', 'DEST'])['DEP_DEL15'].mean().reset_index()
def assign_group(mean_delay):
    if mean_delay > 0.3:
        return 5
    elif 0.25 < mean_delay <= 0.3:
        return 4
    elif 0.2 < mean_delay <= 0.25:
        return 3
    elif 0.15 < mean_delay <= 0.2:
        return 2
    elif 0.1 < mean_delay <= 0.15:
        return 1
    else:
        return 0

# Apply the function to assign groups
route_delays['Group'] = route_delays['DEP_DEL15'].apply(assign_group)
# Composite key 
route_delays['route_key'] = df['ORIGIN'] + '_' + df['DEST']
